...
| `themeConfig.brand`                    | `{ href?, logo?: string \| false, wordmark?: string \| false, productName?: string \| false }` | Optional Header brand overrides. `title` remains the site-level metadata title; `productName` controls the Header's product label. Set `logo`, `wordmark`, or `productName` to `false` to hide it; separators are rendered only between visible parts.                                                                                                                                                                                                                                                                                                                                        |
