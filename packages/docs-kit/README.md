...
| `themeConfig.brand`                    | `{ href?, label?, logo?: string \| false, wordmark?: string \| false, productName?: string \| false }` | Optional Header brand overrides. `title` remains the site-level metadata title; `productName` controls the Header's product label and `label` controls the brand link's accessible name. Set `logo`, `wordmark`, or `productName` to `false` to hide it; separators are rendered only between visible parts.                                                                                                                                                                                                                                                                                                                                        |
