...
AccordionItem supports an `action` prop that allows you to add custom action buttons or icons to the header. The action appears on the right side of the header and can be any ReactNode. You can also set `alwaysShowAction` to make the action visible even when the header is not hovered.
