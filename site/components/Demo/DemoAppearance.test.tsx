...
it.each([
  ...
  (appearance, background, color) => {
    ...
    for (const target of container.querySelectorAll<HTMLElement>('[data-preview]')) {
      ...
      expect(rule?.style.getPropertyValue('background')).toBe(serializeColor(background));
      expect(rule?.style.getPropertyValue('color')).toBe(serializeColor(color));
    }
  },
);
